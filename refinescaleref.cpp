...
namespace scala {
  // ---------------------------------------------------------
  RefineScaleRef::RefineScaleRef(const hkl_unmerge_list& Hkl_list,
                                 ...
                                 const int& Nprocs)
  {
    ...
    hklreflist = &Hklreflist; // pointer to reference list
    ...
    gradientOK = false;
    nprocs=Nprocs;
    //    weighttype = +1;  // default weight type, 1/(Var(I)+Var(Iref))
    weighttype = -1;  //  weight type 1/Var(I)
  }
  // ---------------------------------------------------------
  // ---------------------------------------------------------
  // ---------------------------------------------------------
  floatType RefineScaleRef::targetFn()
  {
    // R = 0.5 * Sum( w (I - g Iref)^2
    if (!gradientOK)  {
...
#pragma omp parallel for default(shared) firstprivate(jcount)           \
  private(jref, jr, myid,                                               \
          invresolsq, index, this_refl, isigiref,                       \
          obs_used, ghl,                                                \
          this_obs, sd, nused, w, g, mnI, l, di)
      ...
      for (jref=0;jref<jcount;++jref) { // ---- loop reflections
        ...
        if (jref_array[jref] >= 0) {
          isigiref = hklreflist->Isig(this_refl.hkl());
          ...
          while ((index = this_refl.next_observation(this_obs)) >= 0) {
            ...
            if (sd > 0.00001) {
              ...
              w = vweight(sd, isigiref); // depending on options
              nused++;
          ...
          if (nused > 0) {
            ...
            for (l=0;l!=nused;++l) {  // loop observations
              ...
              // target function = 0.5 * Sum(w(I-g Iref)^2)
#pragma omp atomic
              target += 0.5 * w * di * di;
#pragma omp atomic
              nobs++;
  ...
  // ---------------------------------------------------------
  floatType    RefineScaleRef::gradientFn(TNT::Vector<floatType>& grad)
  ...
  // ---------------------------------------------------------
  floatType RefineScaleRef::hessianFn(TNT::Fortran_Matrix<floatType>& H,
                                   bool& is_diagonal)
  {
    is_diagonal = false;
  ...
  // ---------------------------------------------------------
  void RefineScaleRef::TargetGradientHessian(bool DoGradient,
                                          bool DoHessian,
                                          TNT::Fortran_Matrix<floatType>& H)
...
#pragma omp parallel for default(shared) firstprivate(jcount, dmnIgldp) \
  private(jref, jr, myid,                                               \
          invresolsq, index, this_refl, isigiref,                       \
          obs_used, ghl, dghldp_obs, dghldp, wdmnIgldp,                 \
          this_obs, sd, nused, w, g, mnI, l, di, ip, \
          jp, ip1)
...
#pragma omp atomic
          ...
          for (l=0;l!=nused;++l) { // loop observations in set
            ...
            for (ip=0;ip!=npar;++ip) {   // Loop parameters
              if (dghldp_obs[l][ip] != 0.0) {
                ...
                // Gradient vector d = - Sum [ whl delI Iref d(ghl)/dp]
                ...
                if (DoHessian) {
                  ...
                  for (jp=0;jp<=ip;jp++) { // loop parameters again (to ip)
                    ...
                    //Hv[ip1+jp] += wgldp * dgldp[jp];  // half matrix
  ...
  // ---------------------------------------------------------
  double RefineScaleRef::vweight(const double& sd, const IsigI& isigiref) const
  // return weight depending on weighttype
  {
    if (weighttype > 0) {
      // variances
      double var = sd*sd + isigiref.sigI()*isigiref.sigI();
      return 1.0/var;
    } else if (weighttype < 0) {
      return 1.0/(sd*sd);
    }
    return 1.0;
  }
  // ---------------------------------------------------------
  void RefineScaleRef::applyShift(TNT::Vector<floatType>& newpar)
  {
    ...
    //    std::cout <<"\n"; //^
    scalemodel->SetParameters(params, nrefpar, false); // no normalisation here
    scalemodel->clearNegativeSecScaleFlag();
    gradientOK = false;
  }
  // ---------------------------------------------------------
  void RefineScaleRef::logCurrent(outStream where, Output& output)
  {
    /*
  ...
  // ---------------------------------------------------------
  std::string  RefineScaleRef::whatAmI(int& i)
  {
    return "Scale "+itos(i);
  }
  // ---------------------------------------------------------
  std::vector<bounds>  RefineScaleRef::getLowerBounds()
  {
  ...
  // ---------------------------------------------------------
  std::vector<bounds>     RefineScaleRef::getUpperBounds()
  {
  ...
  // ---------------------------------------------------------
  TNT::Vector<floatType> RefineScaleRef::getRefinePars()
  {
  ...
  // ---------------------------------------------------------
  TNT::Vector<floatType> RefineScaleRef::getLargeShifts()
  {
  ...
  // ---------------------------------------------------------
  bool1D RefineScaleRef::getRefineMask(protocolPtr protocol)
  {
